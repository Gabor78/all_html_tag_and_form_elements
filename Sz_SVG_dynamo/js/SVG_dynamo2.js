...
function getColor() {
    switch(randomStars(0, 14)) {
        ...
        case 3: return "#88f";
        case 4: return "#ffc";
        case 5: return "url(#grad1)";
        case 6: return "url(#grad2)";
        case 7: return "url(#grad3)";
        case 8: return "#fff";
        case 9: return "#fff";
        case 10: return "#fff";
        case 11: return "#fff";
...
function generateNebulaStars(centerX, centerY, density, maxRadius, numStars) {
    let stars = '';
    for (let i = 0; i < numStars; i++) {
        ...
        const size = randomStars(1, 3) / 10;
        const color = getColor();
        stars += `<circle cx="${x}" cy="${y}" r="${size}" fill="${color}" filter="url(#blr${randomStars(0, 3)})" />`;
    }
    return stars;
}
...
function gen(out_svg="") {
    ...
    const nebulaCenters = [
        { x: svgw * 0.25, y: svgh * 0.3 },
        { x: svgw * 0.75, y: svgh * 0.7 },
        { x: svgw * 0.5, y: svgh * 0.2 },
        { x: svgw * 0.6, y: svgh * 0.6 }
    ];
    ...
    nebulaCenters.forEach(center => {
        result += generateNebulaStars(center.x, center.y, 100, 100, pcs);
    });
    for(let i = 0; i < pcs; i++){
        result += `<circle cx="${randomStars(0, svgw)}" cy="${randomStars(0, svgh)}" r="${randomStars(0, 0.05)}" fill="${getColor()}" filter="url(#blr${randomStars(0, 3)})" />`
    }
