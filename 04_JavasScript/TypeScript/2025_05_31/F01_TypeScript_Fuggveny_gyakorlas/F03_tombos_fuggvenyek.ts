...
function ParatlanokSzama(vizsgaltTomb: number[]): number {
    let paratlanokSzama: number = 0;
...
function LegkisebbParos(vizsgaltTomb: number[]): any {
    let legkisebbParos: number = null;
    for (let i: number = 0; i < vizsgaltTomb.length; i++) {
        if (vizsgaltTomb[i] < legkisebbParos && vizsgaltTomb[i] % 2 == 0) {
