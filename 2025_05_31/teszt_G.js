function FuggvenyVisszajelzoSor(sorszam, tesztNeve, adatBe, elvartEredmeny, fuggvenyhivas) {
    ...
    if (TombOsszehasonlito(elvartEredmeny, fuggvenyhivas) || elvartEredmeny === fuggvenyhivas) {
...
const tesztek = [
    ...
    {
        tesztNeve: "Téglatest felszín",
        adatBe: [2, 3, 4],
        elvartEredmeny: 52,
        fuggvenyhivas: TeglaTestFelszin
    },
    {
    ...
    {
        tesztNeve: "Kocka térfogata",
        adatBe: 2,
        elvartEredmeny: 8,
        fuggvenyhivas: KockaTerfogat
    }
];
...
function TesztFuttato() {
    for (var i = 0; i < tesztek.length; i++) {
        const teszt = tesztek[i];
        try {
            const eredmeny = (
                Array.isArray(teszt.adatBe)  && teszt.fuggvenyhivas.length > 1
            ) 
                ? teszt.fuggvenyhivas(...teszt.adatBe) 
                : teszt.fuggvenyhivas(teszt.adatBe);
